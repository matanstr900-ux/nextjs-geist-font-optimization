...
export default function FillingForm() {
  ...
  const [formData, setFormData] = useState<FillingFormData>({
  ...
  const handleInputChange = (field: keyof FillingFormData, value: string) => {
    setFormData(prev => ({ ...prev, [field]: value }))
  }

  const handleSubmit = () => {
    // Save to localStorage
    const existingData = JSON.parse(localStorage.getItem('fillingData') || '[]')
    const newEntry = {
      ...formData,
    ...
    localStorage.setItem('fillingData', JSON.stringify(existingData))
    ...
    <div className="min-h-screen p-4">
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold">מילוי</h1>
            <p className="text-gray-600">מעקב פעילות מילוי</p>
          </div>
